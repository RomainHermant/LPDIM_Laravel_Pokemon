                            ...
                            <tr class="table-row h4" data-toggle="modal" data-target="#{{$p->nom}}-{{$p->id}}">
                                ...
                                <td class="align-middle">
                                    @foreach ($user->pokemons as $u)
                                        @if ($u->id == $p->id)
                                            <img src="{{asset('storage/images/pokeball-capture.svg')}}" alt="Pokeball vide" width="50" height="50" title="Le pokemon fait partie de mon équipe.">  
                                        @endif
                                    @endforeach
                                    <img src="{{asset('storage/images/pokeball-pas-capture.svg')}}" alt="Pokeball vide" width="50" height="50" title="Le pokemon ne fait pas partie de mon équipe." style="
                                    @foreach ($user->pokemons as $u)
                                        @if ($u->id == $p->id)
                                            display: none;
                                            @break
                                ...
                                <div class="modal-dialog modal-lg">
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            <h5 class="modal-title">
                                                ...
                                                @foreach ($user->pokemons as $u)
                                                    ...
                                                    @endif
                                                @endforeach
                                                <img src="{{asset('storage/images/pokeball-pas-capture.svg')}}" alt="Pokeball vide" width="20" height="20" title="Le pokemon ne fait pas partie de mon équipe." style="
                                                @foreach ($user->pokemons as $u)
                                                    @if ($u->id == $p->id)
                                                        display: none;
                                                        @break
                                        ...
                                        <div class="modal-footer">
                                            @if ($user->pokemons->count() >= 6)
                                                <a title="Supprimer un pokemon de mon équipe pour en ajouter un nouveau." class="text-muted mr-5" href="{{ route('mon-equipe') }}">Mon équipe est pleine</a>
                                            @endif
